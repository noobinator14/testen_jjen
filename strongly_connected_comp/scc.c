...
scc * estimatedStronglyConnectComp(Buffer *out_buffer, Buffer * in_buffer, NodeIndex * out_index, NodeIndex * in_index, int current_out_ind_size, int current_in_ind_size, uint32_t bNode) {
	...
	/*
	inS = malloc((bNode+1)*sizeof(infoSCC));
	if(inS == NULL) {
		free(stack);
		free(visited);
		return NULL;
		...
		if(node1>=0) {
			...
			for(j=0; j<NEIGHB; j++) {
				if(nod1.neighbor[j] == -1) {
					...
					if(SCC->inS[nod1.neighbor[j]].rank < SCC->inS[curN].lowLink) {
						//printf("Switch lowLink of %d from %d to %d\n", curN, SCC->inS[curN].lowLink, SCC->inS[nod1.neighbor[j]].rank);
						SCC->inS[curN].lowLink = SCC->inS[nod1.neighbor[j]].rank;
						//getchar();
					}
				}
			}
				...
				while(1) {
					for (j=0;j<NEIGHB;j++) {
						if (next1.neighbor[j]==-1) {
							...
							if(SCC->inS[next1.neighbor[j]].rank < SCC->inS[curN].lowLink) {
								SCC->inS[curN].lowLink = SCC->inS[next1.neighbor[j]].rank;
							}
						}
					}
					if(found == 1) {
		...
		if(node1>=0) {
			...
			for(j=0; j<NEIGHB; j++) {
				if (nod1.neighbor[j]==-1) {
					...
					//printf("Found ofspirng\n");
					stack[c] = nod1.neighbor[j];
					curN = stack[c];
					curP = c;
					c++;
					found = 1;
					...
					//printf("Found bae of stack\n");
					if(SCC->inS[curN].lowLink > SCC->inS[nod1.neighbor[j]].lowLink) {
						pos = -1;
						for(i=0; i<c; i++) {
							if(stack[i] == nod1.neighbor[j]) {
								pos = i;
							}
							else if(pos > -1) {
								SCC->inS[stack[i]].lowLink = SCC->inS[nod1.neighbor[j]].lowLink;
								if(stack[i] == curN) {
									break;
								}
							}
					...
					while(1) {
						for (j=0;j<NEIGHB;j++) {
							if (next1.neighbor[j]==-1) {
								...
								//printf("OR fo in the stack...\n");
								if(SCC->inS[curN].lowLink > SCC->inS[next1.neighbor[j]].lowLink) {
									pos = -1;
									for(i=0; i<c; i++) {
										if(stack[i] == next1.neighbor[j]) {
											pos = i;
										}
										else if(pos > -1) {
											SCC->inS[stack[i]].lowLink = SCC->inS[next1.neighbor[j]].lowLink;
											if(stack[i] == curN) {
												break;
											}
										}
			...
			if(SCC->inS[stack[curP]].lowLink == SCC->inS[stack[curP]].rank) {
				while(1) {
					SCC->inS[stack[c-1]].lowLink = SCC->comp_counts;
					SCC->inS[stack[c-1]].onS = 0;
					SCC->id_belongs_to_comp[stack[c-1]] = SCC->comp_counts;
					stack[c-1] = 0;
					c--;
	...
	for(j=0; j<SCC->comp_counts; j++) {
		...
		for(i=0; i<bNode+1; i++) {
			...
			if(SCC->id_belongs_to_comp[i] == SCC->comp[j].component_id) {
				SCC->comp[j].node_id[c] = SCC->id_belongs_to_comp[i];
				c++;
			}
		}
		c=0;
